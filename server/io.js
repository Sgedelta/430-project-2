...
const handleTurnInput = async (socket, turnInfo) => {
  ...
  // the game has been informed of our turn, and we don't need to continue this function.
  //   game is waiting.
...
// A function that attempts to start a new game for the player and
//   then returns via socket if the game was made
...
const socketSetup = (app, sessionMiddleware) => {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('RequestGameState', async (roomCode) => {
      const gameState = await returnGameState(roomCode);
